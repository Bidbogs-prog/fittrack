...
-- ---------- COACH CHAT (conversational AI coach, roadmap 1.6) ----------
-- Own rows only. `summary` is a rolling recap of older turns (prompt size
-- control); message `payload` stores guardrail flags for the turn.
...
-- ---------- SAVED MEALS (one meal's rows, re-applied in one tap) ----------
-- A named snapshot of one meal's diary rows for repeated meals/ingredient
...
-- Coach assignment: set = plan handed to one user by an admin (it leaves
-- the global catalogue and becomes visible only to that user + admins).
alter table public.meal_plans
...
-- ---------- PLAN REQUESTS ----------
-- Users ask for a custom plan from /plans; admins fulfil from /admin/requests
-- by assigning an admin-built plan (meal_plans.assigned_to above).
