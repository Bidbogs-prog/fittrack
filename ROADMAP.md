...
- [x] Red-team eval set (`npm run eval:coach`, `src/lib/coach/coach.eval.ts`): 28 graded cases — ED-coded refusals, medical boundary, jailbreaks, grounding/numeric fidelity vs a fixture user, citation discipline, restricted mode — run against the exact production prompt (`src/lib/coach/prompt.ts`), graded by deterministic checks + a strict LLM judge, JSON report to eval-results/. Required before any prompt/brief/context change ships; hits the live API so it never runs in plain `npm test`. Needs `GEMINI_API_KEY` in `.env.local`
...
- [x] Wave 2 — the daily loop: dashboard page, add-food dialog, entry row, activity/water/habits/weight/steps tiles, calendar picker, offline sync, save-meal, food search/picker, barcode scanner, AI insights, micros, macros, pagination (367 keys x 3 locales)
- [x] Action errors returned as `{ error: <status key> }` too, not just URL params — `<ActionError>` is the client counterpart to `<StatusMessage>`. Postgres/Supabase error text collapses to `generic`: it is English from outside the app and not actionable for the reader
- [x] Second guardrail: `src/i18n/message-usage.test.ts` walks the AST, resolves every `useTranslations`/`getTranslations` binding to its namespace (scope-aware — a page's `generateMetadata` and its body bind different ones) and asserts every `t("key")` resolves. next-intl only reports a missing key at render, where the reader sees the key path
- [ ] Remaining surfaces: foods, plans, history, recipes, coach chrome; admin last and arguably never. Enum labels stay English in `nutrition.ts` because `insights.ts`/`report.ts` feed them to Gemini — translate at the render edge via the `activity`/`goal`/`meals` namespaces
