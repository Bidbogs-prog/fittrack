...
- i18n (`src/i18n/`): UI strings live in `messages/{en,fr,ar-MA}.json` — `getTranslations` in server components, `useTranslations`/`useFormatter` in client ones. Never `toLocaleString()` (its locale differs between server and browser, so it mismatches on hydration). Arabic is `ar-MA`; `resolveLocale` aliases legacy `ar` cookies forward. `src/i18n/messages.test.ts` enforces key parity, ICU validity and the dynamic enum lookups — run `npm test` after touching messages.
- Server actions redirect with a status KEY from `src/i18n/status.ts`, never English prose: `redirect(statusUrl("/signup", "error", "passwordTooShort"))`, rendered by `<StatusMessage>`. Add the key to `STATUS_KEYS` and all three catalogues. Supabase auth errors map through `authErrorKey(error)` by `error.code`.
